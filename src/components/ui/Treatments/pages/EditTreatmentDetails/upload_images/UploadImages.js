...
import { getTreatmentFiles } from "../../../../../../api/Treatments";

const UploadImages = ({ imageData, setImageData, oldImages, setOldImages }) => {
  ...
  const handleOldImages = (index) => {
    setOldImages((imgs) => {
      // Create a copy of the previous images array
      const updatedImages = [...imgs];
      const updateImagesData = [...imgs];
      updateImagesData.splice(index, 1);
      // Remove the image at the specified index
            ...
            <div className="flex flex-wrap gap-3">
              {allImages?.map((image, index) => (
                <div key={index} className="relative">
                  <img
                    src={image}
                    className="w-[100px] h-[100px] rounded-15 cursor-pointer"
                    alt={`Image ${index}`}
              ...
              {oldImages?.map((image, index) => (
                <div key={index} className="relative">
                  <img
                    ...
                    className="w-[100px] h-[100px] rounded-15 cursor-pointer"
                    alt={`Image ${index}`}
                    onClick={() => handleViewImage(imageUrls[index])}
                  />
                  <div
                    onClick={() => handleOldImages(index)}
                    className="absolute -top-4 -right-4 cursor-pointer"
                  >
                    <RedClose />
                  </div>
