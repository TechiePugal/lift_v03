...
const ServiceTreatmentInfo = ({ formik, addRow, removeRow, edit }) => {
  ...
  const handleInputChange = (index, event) => {
    try {
      ...
      const balanceTotal = parseFloat(discountedTotal) - parseFloat(amountPaid);
  ...
  const handleQuantityChange = async (index, event) => {
    // Input validation
    if (!event || typeof event !== "number" || event < 1 || !edit) {
    ...
    const newTreatmentTotal =
      (treatment.actualAmount || treatment.amount) * newQuantity;
  ...
  return (
    <div className="shadow-card rounded-15 overflow-x-auto lg:overflow-visible">
      <div
      ...
      <table className=" w-screen md:w-full border-separate border-spacing-y-1 min-w-max ">
        <thead className="">
          <tr>
            ...
            <th className="border-b pb-2 text-bodyBB text-darkgrey">Amount</th>
