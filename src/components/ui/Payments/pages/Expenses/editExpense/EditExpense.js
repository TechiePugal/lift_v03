...
const validationSchema = Yup.object().shape({
  ...
  utility_amount: Yup.string().required("Utility amount is required"),
...
const EditExpense = ({ getUtilityData, handleClose, editData }) => {
  const [loading, setLoading] = useState(false);

  const formik = useFormik({
    initialValues: {
      utility_date: editData?.utility_date,
      utility_name: editData?.utility_name,
      utility_type: editData?.utility_type,
      utility_amount: editData?.utility_amount,
      utility_paid_type: editData?.utility_paid_type,
      utility_next_due_date: editData?.utility_next_due_date,
      utility_remarks: editData?.utility_remarks,
    },
    ...
    onSubmit: (values) => {
      ...
      setLoading(true);
      updateUtility(payload, editData._id)
        .then((response) => {
          showSuccessToast("Utility Edited Successfully");
          getUtilityData();
          handleClose();
        })
        .catch((error) => {
    ...
    <div className="md:w-[500px] lg:w-[835px] p-5">
      <form onSubmit={formik.handleSubmit} className="">
        <div className="grid  lg:grid-cols-2 gap-3 w-full -mt-2 mb-2 ">
          <div className="">
            ...
            <div>
              <DatePicker
                startDate={
                  new Date(formatDateToString(formik?.values?.utility_date))
                }
          ...
          <SelectionInput
            ...
            placeholder={editData?.utility_type}
          ...
          <SelectionInput
            ...
            className={"h-[55px] border-[1.5px] border-[#B9B9B9] shadow-none"}
            placeholder={editData?.utility_paid_type}
            error={
          ...
          <div className="">
            ...
            <div>
              <DatePicker
                startDate={
                  new Date(
                    formatDateToString(formik?.values?.utility_next_due_date)
                  )
                }
                className={"border-[1.5px] border-[#B9B9B9]"}
                error={
        ...
        <TextAreaBox
          ...
          value={formik?.values?.utility_remarks}
          error={
